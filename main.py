...
try:
    ...
    CSS3_COLORS = [
        (webcolors.hex_to_rgb(hex_code), name)
...
class ColorHistory:
    ...
    def setup_ui(self, parent_frame, bg_light):
        ...
        # Add section title
        ttk.Label(history_container, text="COLOR HISTORY", style="Title.TLabel").pack(
            anchor=tk.W
        )
...
class ColorDetectorApp:
    ...
    def __init__(self, root):
        ...
        self.color_processor = (
            ColorProcessor()
        )  # Handles color detection and processing
    ...
    def _configure_styles(self):
        ...
        mono_font = (
            ("Courier New", 11) if self.os_name == "Windows" else ("Monospace", 11)
        )
        ...
        self.style.configure(
            "Title.TLabel", font=bold_font, foreground=self.text_secondary
        )
        self.style.configure(
            "Color.TLabel", font=mono_font, foreground=self.text_primary
        )
        self.style.configure(
            "Status.TLabel",
        ...
        self.style.configure(
            "Accent.TButton", background=self.accent, foreground=self.text_primary
        )
    ...
    def _create_preview_section(self, parent):
        ...
        self.preview_oval = self.color_preview.create_oval(
            10, 10, 130, 130, fill="#ffffff", outline=self.bg_light, width=2
        )
        self.color_preview.pack(pady=5)

        # Section label
        ttk.Label(preview_frame, text="COLOR PREVIEW", style="Title.TLabel").pack(
            pady=(5, 0)
        )
    ...
    def _create_info_section(self, parent):
        ...
        labels_with_copy = [
            (
                "COLOR NAME",
                "name_label",
                lambda: self.safe_copy(self.color_processor.current_name),
            ),
            (
                "HEX CODE",
                "hex_label",
                lambda: self.safe_copy(self.color_processor.current_hex),
            ),
            (
                "RGB VALUES",
                "rgb_label",
                lambda: self.safe_copy(
                    f"{self.color_processor.current_rgb[0]}, {self.color_processor.current_rgb[1]}, {self.color_processor.current_rgb[2]}"
                ),
            ),
        ]
    ...
    def safe_copy(self, text):
        ...
        try:
            ...
            self.status_bar.config(
                text="Clipboard copy failed. Is xclip or xsel installed?"
            )
    ...
    def toggle_freeze(self, event=None):
        ...
        if frozen:
            ...
            self.status_bar.config(
                text="Color frozen. Click 'Unfreeze' to continue detecting."
            )
    ...
    def update_ui_with_current_color(self):
        ...
        self.rgb_label.config(
            text=f"{processor.current_rgb[0]:<3}, {processor.current_rgb[1]:<3}, {processor.current_rgb[2]:<3}"
        )
    ...
    def update_color(self):
        ...
        if not self.color_processor.frozen:
            try:
                ...
                self.status_bar.config(
                    text="PyAutoGUI fail-safe triggered. Please check your cursor position."
                )
            except Exception as e:
                # Handle other detection errors
                self.status_bar.config(
                    text="An error occurred while detecting the color."
                )
